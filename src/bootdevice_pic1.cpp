...
bool
CBootDevice_PIC1::loadBinaryFile(const wxString& path, uint16_t typeHexfile)
{
    unsigned long i;
    bool rv = false;
#ifdef WIN32
    ...
    m_maxUserIDAddr = 0;
    m_bUserIDData   = false;
    m_bUserIDMemory = false; // user id memory should NIT be programmed
                        ...
                        if ((fullAddr < MEMREG_PRG_END) &&
                            ...
                            if (fullAddr > m_maxFlashAddr)
                                m_maxFlashAddr = fullAddr;
                                 ...
                                 ((fullAddr <
                                   MEMREG_CONFIG_START + BUFFER_SIZE_CONFIG))) {
                            ...
                            if (fullAddr > m_maxConfigAddr)
                                m_maxConfigAddr = fullAddr;
                                 ...
                                 ((fullAddr <=
                                   MEMREG_EEPROM_START + BUFFER_SIZE_EEPROM))) {
...
CBootDevice_PIC1::setDeviceInBootMode(void)
{
    ...
    if (USE_DLL_INTERFACE == m_type) {
        ...
        event.vscp_type = ( ID_PUT_BASE_INFO >> 8 );// Special bootloader
        command memset( event.GUID, 0, 16 );                // We use interface
        GUID event.sizeData = 16;                        // Interface GUID
        ...
        if ( VSCP_ERROR_SUCCESS == m_ptcpip->doCmdSendEx( &event ) ) {
            ...
            while( bRun ) {
                ...
                time( &tnow );
                if ( ( unsigned long )( tnow - tstart ) >
        PIC_BOOTLOADER_RESPONSE_TIMEOUT ) { bRun = false;
                }
        ...
        // Read GUID3
        if (VSCP_ERROR_SUCCESS != m_ptcpip->readLevel2Register(VSCP_REG_GUID3,
                                                               0,
                                                               &guid3,
                                                               m_ifguid,
                                                               &m_guid)) {
        ...
        // Read GUID5
        if (VSCP_ERROR_SUCCESS != m_ptcpip->readLevel2Register(VSCP_REG_GUID5,
                                                               0,
                                                               &guid5,
                                                               m_ifguid,
                                                               &m_guid)) {
        ...
        // Read GUID7
        if (VSCP_ERROR_SUCCESS != m_ptcpip->readLevel2Register(VSCP_REG_GUID7,
                                                               0,
                                                               &guid7,
                                                               m_ifguid,
                                                               &m_guid)) {
...
CBootDevice_PIC1::doFirmwareLoad(void)
{
    ...
    if (rv && m_bPrgData) {
        ...
        if (writeDeviceControlRegs(addr)) {
            ...
            for (unsigned long blk = 0; ((blk < nFlashPackets) && bRun);
                ...
                wxStatusStr.Printf(_("Loading flash... %0lX"),
                                   (unsigned long)addr);
                ...
                while (true) {
                    ...
                    if (writeFrimwareSector()) {
                        ...
                        err++;
                        fprintf(stderr,
                                "doFirmwareLoad: Failed to write flash data. "
                                "Will retry...\n");
                    }

                    if (err > PIC_BOOTLOADER_RETRIES) {
                        wxMessageBox(
                          _T("Failed to write flash data to node(s). Fail ")
                          _T("after three resends...±n"));
                        fprintf(
                          stderr,
                          "doFirmwareLoad: Failed to write flash data. [%d]\n",
                          err);
                        rv   = false;
                        bRun = false;
                        break;
    ...
    if (rv && m_bConfigData) {
        ...
        if (writeDeviceControlRegs(addr)) {
            ...
            for (unsigned long blk = 0; ((blk < nConfigPackets) && bRun);
                ...
                wxStatusStr.Printf(_("Loading config memory... %0lX"),
                                   (unsigned long)addr);
                if (!(bRun = pDlg->Update(progress, wxStatusStr))) {
                    wxMessageBox(_T("Aborted by user."));
                    rv   = false;
    ...
    if (rv && m_bEEPROMData) {
        ...
        if (writeDeviceControlRegs(addr)) {
            ...
            for (unsigned long blk = 0; ((blk < nConfigPackets) && bRun);
                ...
                wxStatusStr.Printf(_("Loading EEPROM memory... %0lX"),
                                   (unsigned long)addr);
                if (!(bRun = pDlg->Update(progress, wxStatusStr))) {
                    wxMessageBox(_T("Aborted by user."));
                    rv   = false;
            ...
            wxMessageBox(
              _T("Failed to send control info for EEPROM data to node(s)."));
            rv = false;
        }
    }
    ...
    if (rv) {
        ...
        if (!writeDeviceControlRegs(addr,
                                    ...
                                    ((calc_chksum >> 8) & 0xff))) {
            wxMessageBox(
              _T( "Failed to do finalizing and restart at node(s).Possible checksum error." ));
            fprintf(stderr,
                    "Failed to do finalizing and restart at node(s).Possible "
                    "checksum error.");
            rv   = false;
            bRun = false;
        }
            ...
            fprintf(stderr, "-----------> CMD_CHK_RUN sent\n");
            ...
            pDlg->Update(0, _("Device reset sent."));
            fprintf(stderr, "-----------> CMD_RUN sent.\n");
            ...
            // ||
            //     ( ( USE_TCPIP_INTERFACE == m_type ) && ( 0xfe !=
            //     m_guid.getAt( 15 ) ) ) ) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                wxString str =
                  wxString::Format(_("Trying to verify restart %d/3"), i + 1);
                pDlg->Update(i, str);
                fprintf(stderr, "Trying to verify restart %d\n", i);
            ...
            // Done
            pDlg->Update(3, wxStatusStr);

            if (bReady) {
                wxMessageBox(
                  _T( "Verified that device came out of reset. Firmware loaded." ));
            }
            else {
                wxMessageBox(
                  _T( "Could not verify that device came out of reset." ));
                fprintf(stderr,
                        "Could not verify that device came out of reset.");
                rv = false;
            }
        }
...
bool
CBootDevice_PIC1::writeFrimwareSector(void)
{
    canalMsg msg;
    vscpEventEx event;
    bool rv = true;
...
bool
CBootDevice_PIC1::writeDeviceControlRegs(uint32_t addr,
                                         uint8_t flags,
                                         uint8_t cmd,
                                         uint8_t cmdData0,
                                         uint8_t cmdData1)
{
    ...
    if ((m_pAddr < MEMREG_PRG_END) && (m_pAddr < BUFFER_SIZE_PROGRAM)) {
        ...
        fprintf(stderr, "writeDeviceControlRegs: Invalid memory type. \n");
        return false;
    }
    ...
    if (USE_DLL_INTERFACE == m_type) {
        ...
        event.head       = 0;
        event.vscp_class = 512; // CLASS2.PROTOCOL1
        event.vscp_type  = (ID_PUT_BASE_INFO >> 8) & 0xff;
        m_guid.writeGUID(event.GUID);          // Destination GUID
        event.sizeData = 16 + 8;               // Interface GUID
        m_ifguid.writeGUID(event.data);        // Address node in i/f
    ...
    if (USE_DLL_INTERFACE == m_type) {
        ...
        if (CANAL_ERROR_SUCCESS == m_pdll->doCmdSend(&msg)) {
            ...
            fprintf(
              stderr,
              "writeDeviceControlRegs: Failed to send control command \n");
            rv = false;
        }
    }
    else {
        ...
        if (CANAL_ERROR_SUCCESS == m_ptcpip->doCmdSendEx(&event)) {
            ...
            if (m_bHandshake) {
                ...
                uint8_t err = 0;
                while (!(rv = checkResponseLevel2(ID_RESPONSE_PUT_BASE_INFO))) {
                    err++;
                    fprintf(stderr,
                            "writeDeviceControlRegs: Failed to get response "
                            "from device, still trying %d\n",
                            err);
                    if (err > PIC_BOOTLOADER_RETRIES) {
                        fprintf(stderr,
                                "writeDeviceControlRegs: Failed to get "
                                "response from device. Failing. \n");
                        break;
                    }
                }
            }
...
CBootDevice_PIC1::checkResponseLevel1(uint32_t id)
{
    ...
    while (bRun) {
        ...
        if (CANAL_ERROR_SUCCESS == m_pdll->doCmdReceive(&msg)) {
            fprintf(stderr,
                    "checkResponseLevel1: Reply: %lx == %x\n",
                    msg.id,
                    m_nodeid);
            if (((msg.id & 0xffffff00) == id)) { // correct bootload id

                if ((int)(msg.id & 0xff) == m_nodeid) { // from correct device
...
CBootDevice_PIC1::checkResponseLevel2(uint32_t id)
{
    ...
    while (bRun) {
        ...
        while (m_ptcpip->doCmdDataAvailable()) {
